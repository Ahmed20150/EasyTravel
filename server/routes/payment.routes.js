...
router.get("/verify-payment", async (req, res) => {
  ...
  //   try {
  //     const { products,totalPriceDisplay } = req.body;
  ...
  router.post("/product/create-checkout-session", async (req, res) => {
    try {
        ...
        if (totalPriceDisplay && parseFloat(totalPriceDisplay) > 0) {
            // Use totalPriceDisplay for the checkout
            lineItems.push({
                price_data: {
                    currency: "eur",
                    product_data: {
                    ...
                    unit_amount: Math.round(parseFloat(totalPriceDisplay) * 100), // Convert to cents
                },
                quantity: 1,
            });
        } else {
            // Calculate line items from products
            lineItems = products.map(product => {
                // Validate each product's required fields
                if (!product.giftItem._id || !product.giftItem.name || !product.giftItem.price || !product.giftItem.quantity) {
                    throw new Error("Invalid product data. Each product must have giftItem._id, giftItem.name, giftItem.price, and giftItem.quantity.");
                }
                ...
                return {
                    price_data: {
                        ...
                        unit_amount: Math.round(parseFloat(product.giftItem.price) * 100), // Convert to cents
                    },
