...
router.post("/signUp", upload.single("file"), async (req, res) => {
  try {
    ...
    if (userType === "tourist") {
      ...
      existingUser = await TourGuide.findOne({
        $or: [{ username }, { email }],
      });
    } else if (userType === "advertiser") {
      existingUser = await Advertiser.findOne({
        $or: [{ username }, { email }],
      });
    ...
    if (/\s/.test(email)) {
      return res
        .status(400)
        .json({ message: "Email must not contain spaces." });
    }
    ...
    if (!emailRegex.test(email)) {
      return res.status(400).json({
        message:
          "Invalid email format. Email must be in the format name@[email].com.",
      });
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    const [tourist, tourGuide, advertiser, seller, admin, tourismGoverner] =
      await Promise.all([
    ...
    const user =
      tourist || tourGuide || advertiser || seller || admin || tourismGoverner;
    ...
    if (!isPasswordValid) {
      return res.status(400).json({
        message:
          "Invalid password, Tourist pw is " +
          user.password +
          " while your password is " +
          password,
      });
    }
...
router.post("/changeForgotPassword", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: "No User is associated with this Email Address" });
    }
...
router.post("/sendPaymentEmail", async (req, res) => {
  ...
  const [tourist] = await Promise.all([Tourist.findOne({ email })]);
