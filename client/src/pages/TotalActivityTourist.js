...
const TotalTouristsReport = () => {
  ...
  const fetchAllTourists = async () => {
    try {
      const response = await axios.get('http://localhost:3000/alltourists');
      // setAllTourists(response.data);
  ...
  return (
    <div style={{
      ...
      <h1 style={{ fontSize: '20px', color: '#333', textAlign: 'center', marginBottom: '20px' }}>Tourists Report</h1>
      ...
      {totalTourists !== null && (
        <div style={{
          ...
          {totalTourists > 0 ? (
            ...
            <p style={{ fontSize: '14px', color: '#e74c3c' }}>No tourists came to this activity this month.</p>
          )}
        </div>
      )}
